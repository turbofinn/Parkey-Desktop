...
class ApiService:
    ...
    def otpExitTicket(self, otp, parkingTicketID):
        ...
        headers = {'Content-Type': 'application/json', 'Authorization': "Bearer " + self.env_config.get_token()}
        print(parkingTicketID, otp)
        payload = json.dumps({"exitOTP": otp, "parkingTicketID": parkingTicketID})
        try:
            response = requests.post(BASE_URL + url, headers=headers, data=payload)
            response.raise_for_status()
            return self._get_json_response(response)
    ...
    def exitTicket(self, parkingTicketID):
        ...
        headers = {'Content-Type': 'application/json', 'Authorization': "Bearer " + self.env_config.get_token()}
        payload = json.dumps({"parkingTicketID": parkingTicketID, "employeeID": self.env_config.get_employeeID()})
        try:
            response = requests.post(BASE_URL + url, headers=headers, data=payload)
            response.raise_for_status()
            return self._get_json_response(response)
    ...
    def employeeDetails(self):
        url = "user-management/employee/fetch-employee-info/"+ self.env_config.get_employeeID()
        headers = {'Content-Type': 'application/json', 'Authorization': "Bearer " + self.env_config.get_token()}
