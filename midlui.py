...
class ParkingAppSplash(QMainWindow):
    ...
    def initUI(self):
        ...
        def style_with_shadow(button):
            button.setStyleSheet(radio_style)
            button.setCheckable(True)  # Important to allow :checked style
        ...
        style_with_shadow(self.bike_radio)
        self.car_radio = QPushButton("🚗")
        style_with_shadow(self.car_radio)
        self.truck_radio = QPushButton("🚚")
        style_with_shadow(self.truck_radio)
        self.cycle_radio = QPushButton("🚲")
        style_with_shadow(self.cycle_radio)
        ...
        recent_entry_layout.addWidget(recent_entry_label)
        recent_entry_layout.addWidget(self.recent_entry_list)
        vehicle_layout.addWidget(recent_entry_frame)
        ...
        message_log_layout.addWidget(message_log_label)
        message_log_layout.addWidget(self.message_log_list)
        vehicle_layout.addWidget(message_log_frame)
...
# Connect button events
    ...
    def show_camera_options(self, position):
        ...
        # Always add webcam option
        webcam_action = menu.addAction("Webcam (Default)")

        # Hardened external camera detection
        def detect_external_cameras():
            working_cams = []
            for index in range(1, 5):  # Skip index 0 (internal)
                cap = cv2.VideoCapture(index, cv2.CAP_DSHOW)
                if cap is not None and cap.isOpened():
                    ret, frame = cap.read()
                    cap.release()  # ✅ Always release
                    # Strict check: frame must be valid and not black
                    if (
                        ret
                        and frame is not None
                        and hasattr(frame, 'shape')
                        and frame.shape[0] > 0 and frame.shape[1] > 0
                        and frame.mean() > 10  # Frame should not be completely black
                    ):
        ...
        # Detect real external cams
        external_camera_action = None
        external_cams = detect_external_cameras()
        print("DEBUG >> External Cams:", external_cams)
        ...
        elif external_camera_action and action == external_camera_action:
            self.current_camera_index = external_cams[0]
            self.show_popup(f"Selected external camera (index {self.current_camera_index})")
            if self.cap and self.cap.isOpened():
                self.stop_camera()
            self.start_camera()
